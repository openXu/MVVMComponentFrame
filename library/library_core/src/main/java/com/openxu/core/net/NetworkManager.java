...
import com.openxu.core.net.callback.BaseCallback;
import com.openxu.core.net.callback.DownloadCallback;
import com.openxu.core.net.callback.ResponseCallback;
import com.openxu.core.net.converter.FzyGsonConverterFactory;
import com.openxu.core.net.data.Atta;
...
import com.openxu.core.net.interceptor.BaseInterceptor;
import com.openxu.core.net.interceptor.LoggerInterceptor;
import com.openxu.core.net.rx.BaseOberver;
import com.openxu.core.net.rx.DownLoadNetErrorFunction;
import com.openxu.core.net.rx.DownloadFunction;
import com.openxu.core.net.rx.NetErrorFunction;
import com.openxu.core.net.rx.ParseDataFunction;
import com.openxu.core.net.rx.RetryWhenReset;
import com.openxu.core.net.util.TimeOutDns;
...
public class NetworkManager {
    ...
    static {
        context = XCoreApplication.getApplication().getApplicationContext();
        loadAddress();
    }

    public static void loadAddress() {
//        address = SharedData.getInstance(context).getAddress();
//        port = SharedData.getInstance(context).getPort();
        XLog.w("网络框架初始化：" + address + ":" + port);
...
//                .addInterceptor(new HttpLoggingInterceptor())
        ...
        retrofit = new Retrofit.Builder()
                .client(client)
//                .baseUrl(AppConfig.baseUrl)//"http://114.115.144.251:8001/";
                .baseUrl(getBaseUrl())
    ...
    public static String getBaseUrl() {
        return "http://" + address + (port == 80 ? "" : ":" + port) + "/";
    }
    ...
    public class RequstBuilder {
       public int method;   //请求方式
        ...
        public RequstBuilder serialKey(String serialKey) {
                ...
                .subscribe(new BaseOberver<XResponse>(builder.viewModel, builder.showDialog) {
                    ...
                    public void onNext(XResponse response) {
                        ...
                        try {
                            ...
                            XLog.e("：解析数据错误" + e);
                            if (AppConfig.DEBUG) {
                                XToast.error(getBaseUrl() + "  " + builder.url + "---" + builder.params.toString() + "---" + e.toString());
                            } else {
                    ...
                    public void onError(Throwable e) {
                        XLog.e("请求数据错误：" + e);
                        if (AppConfig.DEBUG) {
                            XFileUtils.writeLogFile(getBaseUrl() + "  " + builder.url + "---" + builder.params.toString() + "---" + e.toString(), "urlLog");
                            builder.callback.onFail(getBaseUrl() + "  " + builder.url + "---" + builder.params.toString() + "---" + e.toString());
                        } else {
