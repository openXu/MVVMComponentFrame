...
public final class XImageUtils {
    ...
    public static Drawable bitmap2Drawable(final Bitmap bitmap) {
        return bitmap == null ? null : new BitmapDrawable(XUtils.getApp().getResources(), bitmap);
    }
    ...
    public static Bitmap getBitmap(@DrawableRes final int resId) {
        Drawable drawable = ContextCompat.getDrawable(XUtils.getApp(), resId);
    ...
    public static Bitmap getBitmap(@DrawableRes final int resId,
                                   ...
                                   final int maxHeight) {
        BitmapFactory.Options options = new BitmapFactory.Options();
        final Resources resources = XUtils.getApp().getResources();
        options.inJustDecodeBounds = true;
    ...
    public static Bitmap renderScriptBlur(final Bitmap src,
        ...
        Bitmap ret = recycle ? src : src.copy(src.getConfig(), true);
        try {
            rs = RenderScript.create(XUtils.getApp());
    ...
    /**
     * 根据屏幕分辨率进行压缩，
     * 分辨率压缩能解决OOM，图片占用内存取决于图片的分辨率  w*h* 单个像素位数
     * 通常用于需要将图片加载到内存中
     *
     * @param uri 图片路径
     */
    public static Bitmap compressByScaleFormUri(Context context, Uri uri) throws IOException {
        InputStream input = context.getContentResolver().openInputStream(uri);

        //每个像素32位，3120*4160*32 / 8 / 1024 = 50700 kb
      /*  Bitmap bitmap1 = BitmapFactory.decodeStream(input, null, null);
        LogUtil.i(TAG, "照片内存大小：" + (bitmap1.getByteCount()/1024) + " kb");
        input.close();
...
//        LogUtil.i(TAG, "照片原始分辨率：" +originalWidth+"*"+originalHeight);
        ...
        int inSampleSizeBig;
        int inSampleSizeSmall;
        int sampleSize;//sampleSize=1表示不缩放
        XLog.i("拍摄照片的宽高：" + originalWidth + "*" + originalHeight +
                "   屏幕宽高：" + screenWidth + "*" + screenHeight);
        //横向图片
        inSampleSizeBig = Math.round((originalWidth > originalHeight ? originalWidth : originalHeight) / ((float) (screenWidth > screenHeight ? screenWidth : screenHeight)));
        inSampleSizeSmall = Math.round((originalWidth > originalHeight ? originalHeight : originalWidth) / ((float) (screenWidth > screenHeight ? screenHeight : screenWidth)));
        sampleSize = (inSampleSizeSmall > inSampleSizeBig ? inSampleSizeSmall : inSampleSizeBig) + 1;
//        LogUtil.i(TAG, "长的一边缩放比："+inSampleSizeBig+"   短的一边缩放比"+inSampleSizeSmall+"  按照大的缩放"+sampleSize);

        options.inJustDecodeBounds = false;
        options.inSampleSize = sampleSize;//设置缩放比例
        options.inDither = true;
        //一般假如对图片没有透明度要求的话，可以改成RGB_565，相比ARGB_8888将节省一半的内存开销
//        bitmapOptions.inPreferredConfig = Bitmap.Config.ARGB_8888;//32位/像素  1560*2080 * 32 /8 /1024 = 12675kb
        options.inPreferredConfig = Bitmap.Config.RGB_565;    //16位/像素    1560*2080 * 16 /8 /1024 = 6337.5 kb
        input = context.getContentResolver().openInputStream(uri);
        Bitmap bitmap = BitmapFactory.decodeStream(input, null, options);
        input.close();
        XLog.i("分辨率压缩后图片的大小" + (bitmap.getByteCount() / 1024)
                + "kb   宽高 " + bitmap.getWidth() + "*" + bitmap.getHeight());
        return bitmap;
    }
