...
import com.openxu.core.zxing.google.camera.CameraManager;
...
public class QRCodeReaderView extends SurfaceView
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        decodeHints = new Hashtable<DecodeHintType, Object>(3);
...
//        decodeHints.put(DecodeHintType.POSSIBLE_FORMATS, decodeFormats);
        decodeFrameTask = new DecodeFrameTask(this, null);
    ...
    private  class DecodeFrameTask extends AsyncTask<byte[], Void, Result> {
        ...
        protected Result doInBackground(byte[]... params) {
            ...
            int height=0;
            //modify here
            byte[] rotatedData = new byte[params[0].length];
            for (int y = 0; y < view.mPreviewHeight; y++) {
                for (int x = 0; x < view.mPreviewWidth ; x++)
                    rotatedData[x * view.mPreviewHeight + view.mPreviewHeight - y - 1] = params[0][x + y * view.mPreviewWidth];
            }
            int tmp = view.mPreviewWidth ; // Here we are swapping, that's the difference to #11
            width  = view.mPreviewHeight;
            height = tmp;
            final PlanarYUVLuminanceSource source =
                    view.mCameraManager.buildLuminanceSource(rotatedData, width,
                            height);
...
//                view.mQRCodeReader.reset();
        ...
        private PointF[] transformToViewCoordinates(QRCodeReaderView view,
            ...
            Orientation orientation =
                    orientationDegrees == 90 || orientationDegrees == 270 ? Orientation.LANDSCAPE
                            : Orientation.PORTRAIT;
            Point viewSize = new Point(view.getWidth(), view.getHeight());
            Log.e("TAG", "当前的黄点的宽度===="+view.getWidth()+"    高度==="+view.getHeight());
            Point cameraPreviewSize = view.mCameraManager.getPreviewSize();
            int y= cameraPreviewSize.y;
            cameraPreviewSize.y=cameraPreviewSize.x;
            cameraPreviewSize.x=y;
            Log.e("TAG", "当前的黄点的pointx===="+cameraPreviewSize.x+"    y==="+cameraPreviewSize.y);
