 ...
 * Description: 通用的下拉刷新、上拉加载列表页面
 ...
 public class ListFragment extends BaseListFragment<CoreFragmentBaseListBinding, RefreshListViewModel, RefreshListItem>
 * 注意：
 * 1、如果使用通用布局（不重写getContentView(Bundle savedInstanceState)）,Binding泛型请传入CoreFragmentBaseListBinding
 ...
 * 1、覆盖initListPageParams()完成基本初始化操作：
 ...
 itemClickId.add(R.id.tv_select);                 //item中某些子控件需要绑定点击事件，需要添加到itemClickId中
 //设置刷新机制
 binding.refreshLayout.setEnableRefresh(true);
 binding.refreshLayout.setEnableLoadMore(true);
 //分割线
 binding.recyclerView.addItemDecoration(new CommandItemDecoration(getContext(),LinearLayoutManager.VERTICAL,
 getResources().getColor(R.color.line_bg_color),
 (int)(getResources().getDimension(R.dimen.line_space))));
 }
  *
 ...
 *
 */
public abstract class XBaseListFragment<V extends ViewDataBinding, VM extends XBaseViewModel, D> extends XBaseFragment<V, VM> {
    ...
    protected boolean mEnableRefresh, mEnableLoadMore;
    //需要初始化设置
    protected int PageSize = 30;   //每页记录数
    protected int itemLayout, extrHeaderLayout, extrFooterLayout;
    protected View extrHeaderView, extrFooterView;
    ...
    public void initView() {
        initListPageParams();
        initListAdapter();
        viewModel.listNetError.observe(this, aBoolean -> {
            if (aBoolean) {
                responseData(null);
            }
        });
    ...
    /**
     * adapter绑定数据，默认支持databinding，但是有一些特殊的可以通过该方法绑定
     */
    protected void convertView(ViewHolder holder, D data, int position) {
    }
    ...
    protected void initListAdapter() {
        ...
        if (extrHeaderLayout != 0 && null != extrHeader)
            headerBinding = DataBindingUtil.inflate(LayoutInflater.from(getContext()), extrHeaderLayout, extrHeader, true);
        XLog.w("底部：" + extrFooterLayout + "    extrFooter=" + extrFooter);
        if (extrFooterLayout != 0 && null != extrFooter) {
            footerBinding = DataBindingUtil.inflate(LayoutInflater.from(getContext()), extrFooterLayout, extrFooter, true);
        }

        if (null != extrHeaderView && null != extrHeader)
            extrHeader.addView(extrHeaderView);
        if (null != extrFooterView && null != extrFooter)
            extrFooter.addView(extrFooterView);
...
//        refreshFooter.setProgressResource(R.drawable.ic_progress);//设置图片资源
        ...
        adapter = new CommandRecyclerAdapter<D>(getContext(), itemLayout, null) {
            @Override
            public void convert(ViewHolder holder, D data, int position) {
                ...
                XBaseListFragment.this.convertView(holder, data, position);
            ...
            public void onItemClick(D data, int position) {
                XBaseListFragment.this.onItemClick(data, position);
            }

            @Override
            public void onItemLongClick(D data, int position) {
                XBaseListFragment.this.onItemLongClick(data, position);
            }
        };
...
//            refreshLayout.finishLoadMoreWithNoMoreData();
    ...
    /**
     * 设置数据（MessageFragment使用：由于消息数据分页方式和网络请求不同）
     */
...
//            refreshLayout.finishLoadMoreWithNoMoreData();
    ...
    public void showProgressDialog() {
        ...
        if (mEnableRefresh)   //如果不支持下拉刷新，显示让BaseActivity显示对话框
            refreshLayout.autoRefresh(0, 1, 1, true);
        else
