...
import com.openxu.ui.ActivitySetUser;
import com.openxu.ui.ImageBrowserActivity;
import com.openxu.ui.MyApplication;
import com.openxu.utils.FaceTextUtils;
import com.openxu.utils.TimeUtil;
...
public class MessageChatAdapter extends BaseListAdapter<BmobMsg> {
	...
	public View bindView(final int position, View convertView, ViewGroup parent) {
		...
		if(!TextUtils.isEmpty(avatar)){//加载头像-为了不每次都加载头像
		...
		iv_avatar.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				if(getItemViewType(position) == TYPE_RECEIVER_TXT 
					...
					Intent intent =new Intent(mContext,ActivitySetUser.class);
					mContext.startActivity(intent);
				}
		...
		if(position>=1){
			BmobMsg preItem = list.get(position-1);
			if(TimeUtil.showTime(Long.parseLong(item.getMsgTime()), Long.parseLong(preItem.getMsgTime()))){
