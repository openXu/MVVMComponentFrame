...
public class VoicePlayerImpl {
	...
	private static VoicePlayerImpl instance;


	public static VoicePlayerImpl getInstance(Context context){
		if(instance==null)
			instance = new VoicePlayerImpl(context);
		return instance;
	}

	private VoicePlayerImpl(Context context) {
...
//		mTts.setParameter(SpeechConstant.VOLUME, "5");//设置音量，范围0~100  
	...
	private SynthesizerListener mSynListener = new SynthesizerListener(){  
	    ...
	    public void onCompleted(SpeechError error) {
	    	if(error!=null){
				XToast.error("播报错误"+error.getErrorCode()+" "+error.getMessage());
				XLog.i( "播报错误"+error.getErrorCode()+" "+error.getMessage());
	    	}
	    }  
	    //缓冲进度回调  
