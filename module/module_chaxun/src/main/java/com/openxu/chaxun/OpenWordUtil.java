...
public class OpenWordUtil {
     ...
     * @return
     */  
	public static String formatSent(Pf pf, String sents, String english) {
		String result = "";
		try{
			...
			if(sentsList!=null&&sentsList.size()>0){
				for(int i=0;i<sentsList.size();i++){
			        ...
			        String orig = m.replaceAll("<font color=\""+pf.sents_en_color+"\">"+english+"</font>");
     ...
     * @return 
     */  
	public static String formatSentZh(Pf pf, String sents, String zh) {  
		String result = "";
		try{
			List<OpenSent> sentsList = JSON.parseArray(sents, OpenSent.class);
			String[] big = new String[]{"一","二","三","四","五"};
				...
				for(int i=0;i<sentsList.size();i++){
					...
					if(!TextUtils.isEmpty(sentTran)){
						if(sentTran.startsWith(zh)){
							tranStr += ("<font color=\""+pf.sents_en_color+"\">"+zh+"</font>");
						}
						...
						for(int j=0;j<trans.length;j++){
							...
							if(!TextUtils.isEmpty(tran)){
								if(j==trans.length-1)
									...
									tranStr += ("<font color=\"#74787c\">"+tran+"</font>"+"<font color=\""+pf.sents_en_color+"\">"+zh+"</font>");
							}
						}
	...
	public static String getOnePart(String parts) {
		...
		if(!TextUtils.isEmpty(parts)){
			...
			if(partsarray!=null){
				...
				XLog.i( "获取意思："+result);  //获取意思：n.["饥荒","饥饿","极度缺乏"
				String ps[] = result.split("\\.\\[");
				XLog.i( "获取意思："+ps.length);
				if(ps.length>=2){
					result = ps[0]+".";   //n.
